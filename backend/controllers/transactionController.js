...
export const getAllTransactionController = async (req, res) => {
  try {
    const { userId, frequency, startDate, endDate, type } = req.body;

    console.log("getTransactions body:", req.body);
    ...
    if (frequency && frequency !== "all" && frequency !== "custom") {
      const days = Number(frequency);
      if (!Number.isNaN(days)) {
        const fromDate = new Date();
        fromDate.setDate(fromDate.getDate() - days);
        query.date = { $gte: fromDate, $lte: new Date() };
      }
    }
...
export const updateTransactionController = async (req, res) => {
  try {
    ...
    if (date) {
      transactionElement.date = new Date(date); // Convert to Date
    }
